...
-- Description:	Перезагрузка определенной смены на определенной кассе (копипаст из генерального обмена)
-- =============================================
CREATE PROCEDURE [cass].[p_Перезагрузка_смены]
@Код_кассы int,
@ИД_смены int
AS
BEGIN
	...
	where name like 'CassLoader%' --kill 53
	...
	set @IP_адрес     = (select IP_адрес     from cass.t_dim_Кассы where Код_кассы = @Код_кассы)
	set @Код_магазина = (select Код_магазина from cass.t_dim_Кассы where Код_кассы = @Код_кассы)
	set @Включена     = (select Включена     from cass.t_dim_Кассы where Код_кассы = @Код_кассы)
