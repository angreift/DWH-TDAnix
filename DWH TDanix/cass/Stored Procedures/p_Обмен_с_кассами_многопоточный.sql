...
BEGIN
	...
	/* Не проверяем на запуск других гранзакий, так как потоки изолированы друг от друга
	...
	*/
	...
	-- Используем одноразовые временные таблицы

	set @str = '

		create table ##t_raw_Кассовые_документы_%ИД_обмена% (
		...
		create table ##t_raw_Оплаты_%ИД_обмена% (
			[ИД_документа] [int] NOT NULL,
			[Код_типа_оплаты] [int] NOT NULL,
			[Сумма_оплаты] [money] NOT NULL
		)

		create table ##t_raw_Позиции_документа_%ИД_обмена% (
		...
		create table ##t_raw_Пользователи_%ИД_обмена% (
			[Код_кассира] [nvarchar](30) NOT NULL,
		...
		create table ##t_raw_Скидки_%ИД_обмена% (
		...
		create table ##t_raw_Смены_%ИД_обмена% (
			[ИД_смены] [int] NOT NULL,
		...
		create table ##t_raw_Сторнированные_позиции_%ИД_обмена% (
	...
	from
		cass.t_dim_Кассы
	where
		cass.t_dim_Кассы.Код_кассы >= @НачалоДиапазона and cass.t_dim_Кассы.Код_кассы <= @КонецДиапазона
	...
	begin
		...
		*/
		...
		set @str = 'truncate table ##t_raw_Пользователи_%ИД_обмена%';
		set @str = replace(@str, '%ИД_обмена%', @ИД_обмена);
		exec(@str);

		set @str = '
			insert into 
				##t_raw_Пользователи_%ИД_обмена% (
					Код_кассира,
					Логин_пользователя,
					Имя_пользователя,
		...
		begin try
			set @str = '
				...
				using ##t_raw_Пользователи_%ИД_обмена%
				on (cass.t_dim_Пользователи_на_кассах.Код_кассы = %Код_кассы% and 
					cass.t_dim_Пользователи_на_кассах.Код_кассира = ##t_raw_Пользователи_%ИД_обмена%.Код_кассира)
				when matched then
					update set	cass.t_dim_Пользователи_на_кассах.Логин_пользователя    = ##t_raw_Пользователи_%ИД_обмена%.Логин_пользователя, 
								cass.t_dim_Пользователи_на_кассах.Имя_пользователя      = ##t_raw_Пользователи_%ИД_обмена%.Имя_пользователя, 
								cass.t_dim_Пользователи_на_кассах.Запрещена_авторизация = ##t_raw_Пользователи_%ИД_обмена%.Запрещена_авторизация, 
								cass.t_dim_Пользователи_на_кассах.Должность             = ##t_raw_Пользователи_%ИД_обмена%.Должность, 
								cass.t_dim_Пользователи_на_кассах.ИНН                   = ##t_raw_Пользователи_%ИД_обмена%.ИНН
				when not matched then
					insert (Код_кассы, Код_кассира, Логин_пользователя, Имя_пользователя, Запрещена_авторизация, Должность, ИНН)
					values (
						%Код_кассы%,
						##t_raw_Пользователи_%ИД_обмена%.Код_кассира,
						##t_raw_Пользователи_%ИД_обмена%.Логин_пользователя,
						##t_raw_Пользователи_%ИД_обмена%.Имя_пользователя,
						##t_raw_Пользователи_%ИД_обмена%.Запрещена_авторизация, 
						##t_raw_Пользователи_%ИД_обмена%.Должность, 
						##t_raw_Пользователи_%ИД_обмена%.ИНН
					);
				';
			set @str = replace(@str, '%ИД_обмена%', @ИД_обмена);
			set @str = replace(@str, '%Код_кассы%', cast(@Код_кассы as nvarchar));
			exec(@str);
		...
		set @str = 'truncate table ##t_raw_Смены_%ИД_обмена%';
		set @str = replace(@str, '%ИД_обмена%', @ИД_обмена);
		exec(@str);

		set @str = '
			insert into 
				##t_raw_Смены_%ИД_обмена% (
					ИД_смены,
					Номер_смены,
					Код_кассира,
		...
		while (select count(*) from @t_raw_Смены) > 0 begin
			...
			set @ИД_смены                         = (select top 1 ИД_смены                   from @t_raw_Смены);
			set @Номер_смены                      = (select Номер_смены                      from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Код_кассира                      = (select Код_кассира                      from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Дата_время_начала_смены          = (select Дата_время_начала_смены          from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Дата_время_окончания_смены       = (select Дата_время_окончания_смены       from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Номер_первого_чека_в_смене       = (select Номер_первого_чека_в_смене       from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Номер_последнего_чека_в_смене    = (select Номер_последнего_чека_в_смене    from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_продажи                    = (select Сумма_продажи                    from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_выручки                    = (select Сумма_выручки                    from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_в_денежном_ящике           = (select Сумма_в_денежном_ящике           from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Признак_изменения_данных         = (select Признак_изменения_данных         from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Дата_время_открытия_первого_чека = (select Дата_время_открытия_первого_чека from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_продажи_наличные           = (select Сумма_продажи_наличные           from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_продажи_безналичные        = (select Сумма_продажи_безналичные        from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_продажи_прочие             = (select Сумма_продажи_прочие             from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_выручки_наличные           = (select Сумма_выручки_наличные           from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_выручки_безналичные        = (select Сумма_выручки_безналичные        from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_возвратов                  = (select Сумма_возвратов                  from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_возвратов_наличные         = (select Сумма_возвратов_наличные         from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Сумма_возвратов_безналичные      = (select Сумма_возвратов_безналичные      from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Количество_чеков_продажи         = (select Количество_чеков_продажи         from @t_raw_Смены where ИД_смены = @ИД_смены);
			set @Количество_чеков_возврата        = (select Количество_чеков_возврата        from @t_raw_Смены where ИД_смены = @ИД_смены);
			delete from @t_raw_Смены where ИД_смены = @ИД_смены;
			...
			set @TransactionName = left('CL_%S%_' + cast(@Код_кассы as nvarchar) + '_' + cast(@Номер_смены as nvarchar), 32);
			set @TransactionName = replace(@TransactionName, '%S%', @ИД_обмена);
			...
			set @str = '
				truncate table ##t_raw_Кассовые_документы_%ИД_обмена%
				truncate table ##t_raw_Позиции_документа_%ИД_обмена%
				truncate table ##t_raw_Пользователи_%ИД_обмена%
				truncate table ##t_raw_Скидки_%ИД_обмена%
				truncate table ##t_raw_Сторнированные_позиции_%ИД_обмена%
				truncate table ##t_raw_Оплаты_%ИД_обмена%
			';
			...
			begin try
				set @str = '
				insert into
					##t_raw_Кассовые_документы_%ИД_обмена% (
						ИД_документа,
						ИД_смены,
						Номер_чека,
						Код_кассира,
			...
			begin try
				set @str = '
				insert into
					##t_raw_Позиции_документа_%ИД_обмена% (
						ИД_позиции,
						ИД_документа,
						Код_кассира,
			...
			begin try
				set @str = '
				insert into
					##t_raw_Сторнированные_позиции_%ИД_обмена% (
			...
			set @str = '
				...
				from
					##t_raw_Кассовые_документы_%ИД_обмена% RDoc
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_n  on RDoc.ИД_документа = RM_n.ИД_документа  and RM_n.Код_типа_оплаты  in (1)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_t  on RDoc.ИД_документа = RM_t.ИД_документа  and RM_t.Код_типа_оплаты  in (3)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_s  on RDoc.ИД_документа = RM_s.ИД_документа  and RM_s.Код_типа_оплаты  in (15)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_nt on RDoc.ИД_документа = RM_nt.ИД_документа and RM_nt.Код_типа_оплаты in (101)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_c  on RDoc.ИД_документа = RM_c.ИД_документа  and RM_c.Код_типа_оплаты  in (105)
			
			';
			set @str = REPLACE(@str, '%ИД_обмена%', @ИД_обмена);
			set @str = REPLACE(@str, '%Код_кассы%', @Код_кассы);
			set @str = REPLACE(@str, '%ИД_смены%', @ИД_смены);
			...
			set @str = '
				...
				from
					##t_raw_Позиции_документа_%ИД_обмена% RG
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_n  on RG.ИД_документа = RM_n.ИД_документа  and RM_n.Код_типа_оплаты  in (1)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_t  on RG.ИД_документа = RM_t.ИД_документа  and RM_t.Код_типа_оплаты  in (3)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_s  on RG.ИД_документа = RM_s.ИД_документа  and RM_s.Код_типа_оплаты  in (15)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_nt on RG.ИД_документа = RM_nt.ИД_документа and RM_nt.Код_типа_оплаты in (101)
				left join
					##t_raw_Оплаты_%ИД_обмена% RM_c  on RG.ИД_документа = RM_c.ИД_документа  and RM_c.Код_типа_оплаты  in (105)
				left join
					##t_raw_Кассовые_документы_%ИД_обмена% RDoc on RG.ИД_документа = RDoc.ИД_документа
			...
			set @str = '
				...
				from
					##t_raw_Скидки_%ИД_обмена% RD
				left join
					##t_raw_Позиции_документа_%ИД_обмена% RG on RD.ИД_позиции = RG.ИД_позиции
