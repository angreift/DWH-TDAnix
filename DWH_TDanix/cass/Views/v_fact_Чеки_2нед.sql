CREATE VIEW cass.v_fact_Чеки_2нед
AS
...
FROM            cass.t_fact_Чеки AS c INNER JOIN
                        ...
                        where c.Флаг_закрытия_чека in (1, 2) and c.Флаг_закрытия_чека is not null and  c.Дата_время_закрытия_чека>=dateadd(day,-14,GETDATE()) --Сюда попадаю только закрытые чеки. Незакрытые чеки используются в сторно
