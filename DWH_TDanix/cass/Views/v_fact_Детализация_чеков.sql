...
SELECT               cass.t_fact_Детализация_чеков.Код_кассы, cass.t_fact_Детализация_чеков.Дата_время_добавления_позиции, cass.t_fact_Детализация_чеков.Способ_добавления_позиции, 
                         ...
                         COALESCE
                             ((SELECT        TOP (1) Признак
                                 ...
                                 WHERE        (Дата <= cass.t_fact_Детализация_чеков.Дата_добавления_позиции) AND (Код_товара = cass.t_fact_Детализация_чеков.Код_товара) AND (Код_магазина = cass.t_dim_Кассы.Код_магазина)), 0) 
                         AS Признак,
