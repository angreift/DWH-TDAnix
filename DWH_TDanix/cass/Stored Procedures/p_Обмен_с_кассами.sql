...
BEGIN
	...
	begin
		...
		while (select count(*) from cass.t_raw_Смены) > 0 begin
			...
			if (select count(*) from cass.t_fact_Смены_на_кассах where cass.t_fact_Смены_на_кассах.Составной_код_смены = @Составной_код_смены) = 0 begin
				set @Флаг_загрузки_смены = 1;
				set @msg = concat('Смена не существует в хранилище. Она будет загружена. Код_кассы: ', @Код_кассы, ', IP: ', @IP_адрес, ', Код_магазина: ', @Код_магазина, ', ИД_смены: ', @Номер_смены);
				exec [dbo].[p_Сообщить_в_общий_журнал] 3, @object_name, @msg;
			end

			if (@Номер_смены                      <> (select Номер_смены                      from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Составной_код_кассира            <> (select Составной_код_кассира            from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Дата_время_начала_смены          <> (select Дата_время_начала_смены          from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Дата_время_окончания_смены       <> (select case when Дата_время_окончания_смены is null then cast('20000101' as datetime) else Дата_время_окончания_смены end from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Номер_первого_чека_в_смене       <> (select Номер_первого_чека_в_смене       from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Номер_последнего_чека_в_смене    <> (select Номер_последнего_чека_в_смене    from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_продажи                    <> (select Сумма_продажи                    from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_выручки                    <> (select Сумма_выручки                    from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_в_денежном_ящике           <> (select Сумма_в_денежном_ящике           from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			  -- (@Признак_изменения_данных         <> (select Признак_изменения_данных         from cass.t_fact_Смены_на_кассах where Код_кассы = @Составной_код_смены)) or // Не рентабельно мониторить это поле
			   (@Дата_время_открытия_первого_чека <> (select Дата_время_открытия_первого_чека from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_продажи_наличные           <> (select Сумма_продажи_наличные           from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_продажи_безналичные        <> (select Сумма_продажи_безналичные        from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_продажи_прочие             <> (select Сумма_продажи_прочие             from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_выручки_наличные           <> (select Сумма_выручки_наличные           from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_выручки_безналичные        <> (select Сумма_выручки_безналичные        from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_возвратов                  <> (select Сумма_возвратов                  from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_возвратов_наличные         <> (select Сумма_возвратов_наличные         from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Сумма_возвратов_безналичные      <> (select Сумма_возвратов_безналичные      from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Количество_чеков_продажи         <> (select Количество_чеков_продажи         from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) or
			   (@Количество_чеков_возврата        <> (select Количество_чеков_возврата        from cass.t_fact_Смены_на_кассах where Составной_код_смены = @Составной_код_смены)) begin
				set @Флаг_загрузки_смены = 1;
				set @msg = concat(
					'Смена в хранилище существует, но не совпадают данные. Она будет перезагружена. Код_кассы: ', @Код_кассы, 
					', IP: ', @IP_адрес, 
					', Код_магазина: ', @Код_магазина, 
					', ИД_смены: ', @Номер_смены
					);
