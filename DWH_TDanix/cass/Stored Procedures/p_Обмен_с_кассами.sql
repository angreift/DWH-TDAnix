	...
	begin
		...
		while (select count(*) from cass.t_raw_Смены) > 0 begin
			...
			if (@forceWorkshift = 0) begin
				-- Если явно не указана смена для переснятия, то перезаписываются только незакрытые смены
				if (@Дата_время_окончания_смены is null) set @Флаг_загрузки_смены = 1;
				set @msg = concat('Смена существует в хранилище, но она не закрыта. Перезапишем. Код_кассы: ', @Код_кассы, ', IP: ', @IP_адрес, ', Код_магазина: ', @Код_магазина, ', ИД_смены: ', @Номер_смены);
				exec [dbo].[p_Сообщить_в_общий_журнал] 3, @object_name, @msg;
			end
