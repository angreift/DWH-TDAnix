...
BEGIN
	...
	*/
	...
	-- Если за день существует ошибочная загрузка смены и ни одной успешной

	drop table if exists #Неопрошенные_кассы

	create table #Неопрошенные_кассы (
		Код_кассы int,
		IP_Адрес nvarchar(15),
		Включена bit,
		Код_магазина int,
		Группа_магазина nvarchar(100),
		Наименование_магазина nvarchar(100),
		Сообщение nvarchar(max)
	)
	
	insert into #Неопрошенные_кассы
	Select 
		к.[Код_кассы]
		,к.[IP_Адрес]
		,к.[Включена]
	...
	from
		cass.v_dim_Кассы as к with (nolock)
	where
		к.Код_кассы in (
			select distinct
				Код_кассы
			from
				cass.t_j_История_загрузки_смен_на_кассе и
			where
				cast(И.Дата_время_начала_загрузки as date) = dateadd(day, -1, cast(getdate() as date)) and Дата_время_окончания_загрузки is null and not exists (
					select * from cass.t_j_История_загрузки_смен_на_кассе и2 where cast(и2.Дата_время_начала_загрузки as date) = dateadd(day, -1, cast(getdate() as date)) and и2.Дата_время_окончания_загрузки is not null
		))

	-- Ошибки касс из журнала
	insert into #Неопрошенные_кассы
	select 
		top 1
		к.[Код_кассы] as [Код_кассы]
		...
		,ж.Сообщение
	from 
		cass.t_j_Ошибки_опроса_касс ж with (nolock)
	left join
		[cass].[v_dim_Кассы] к with (nolock) on ж.Код_кассы = к.Код_кассы		
	where
		 к.Код_кассы not in (select Код_кассы from #Неопрошенные_кассы) and к.Код_кассы in (
			select distinct Код_кассы from cass.t_j_Ошибки_опроса_касс ж2 where cast(ж.Дата_время_события as date) = dateadd(day, -1, cast(getdate() as date))
		)
	order by
		ж.Код_события desc
