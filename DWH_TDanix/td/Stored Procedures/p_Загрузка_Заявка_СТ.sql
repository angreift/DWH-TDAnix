...
-- Description:	Загрузка данных шапок и строк документов Заявка СТ
-- =============================================
CREATE PROCEDURE [td].[p_Загрузка_Заявка_СТ]
AS
BEGIN
	set noCount on;
	...
	Declare @Составной_код_заявки_СТ nvarchar(15),
		@Дата_заявки_СТ date
	...
	Declare @Код_товара bigint,
		...
		@План decimal(12,3),
		@Заказ decimal(14,3),
		@Корректировка decimal(14,3),
		@Заказ_выпечка decimal(14,3),
		@Заказ_хознужды decimal(14,3),
		@В_пути decimal(14,3),
		@Цена decimal(12,2),
	...
	from
		[td].[t_raw_Данные_Заявка_СТ_шапки] with (nolock)
	...
	while (select count(*) from @load_pr) > 0 begin
		...
		if (select count(*) from [td].[t_raw_Данные_Заявка_СТ_шапки] with (nolock) where
				Дата_выгрузки = @_дата_выгрузки and
				...
				) = 0 or (select count(*) from [td].[t_raw_Данные_Заявка_СТ_строки] with (nolock) where
				Дата_выгрузки = @_дата_выгрузки and
				Код_магазина = @_код_магазина and
				Выгрузка_из_ЗОД = @_выгрузка_зод and
				Отпечаток_времени = @_отпечаток_времени and
			...
			if @_дата_выгрузки < (dateAdd(day, - 2, getDate())) begin
				delete from td.t_raw_Данные_Заявка_СТ_строки  where Дата_выгрузки = @_дата_выгрузки and
																			  Код_магазина = @_код_магазина and
																			  Выгрузка_из_ЗОД = @_выгрузка_зод and
																			  Отпечаток_времени = @_отпечаток_времени;
				delete from td.t_raw_Данные_Заявка_СТ_шапки   where Дата_выгрузки = @_дата_выгрузки and
																			  Код_магазина = @_код_магазина and
																			  Выгрузка_из_ЗОД = @_выгрузка_зод and
																			  Отпечаток_времени = @_отпечаток_времени;
		...
		Delete from [td].t_raw_Данные_Заявка_СТ_шапки where
			Дата_выгрузки = @_дата_выгрузки and
			Код_магазина = @_код_магазина and
			Выгрузка_из_ЗОД = @_выгрузка_зод and
			Отпечаток_времени = @_отпечаток_времени and
			[Data] = '01' + format(@_дата_выгрузки, 'yyMMdd') + case when @_выгрузка_зод = 1 then 'Z' else 'D' end + right('0000' + cast(@_код_магазина as nvarchar), 4) + right('000000' + cast(@_отпечаток_времени as nvarchar), 6) + 'finish';
		Delete from [td].t_raw_Данные_Заявка_СТ_строки where
			Дата_выгрузки = @_дата_выгрузки and
			Код_магазина = @_код_магазина and
			Выгрузка_из_ЗОД = @_выгрузка_зод and
			Отпечаток_времени = @_отпечаток_времени and
		...
		if @_выгрузка_зод = 1 begin
			delete from td.t_fact_Заявка_СТ_шапки where Дата_заявки_СТ >= dateFromParts(year(@_дата_прошлый_месяц), month(@_дата_прошлый_месяц), 1) and Дата_заявки_СТ <= @_дата_выгрузки and Код_магазина = @_код_магазина
			...
			delete from td.t_fact_Заявка_СТ_шапки where Дата_заявки_СТ = @_дата_выгрузки and Код_магазина = @_код_магазина
		...
		insert into @Данные ([data]) select [Data] from [td].t_raw_Данные_Заявка_СТ_шапки where Код_магазина = @_код_магазина and
		                           Дата_выгрузки = @_дата_выгрузки and
								   Выгрузка_из_ЗОД = @_выгрузка_зод and
								   Отпечаток_времени = @_отпечаток_времени;
		insert into @Данные_строки ([data]) select [Data] from [td].t_raw_Данные_Заявка_СТ_строки where Код_магазина = @_код_магазина and
		                           Дата_выгрузки = @_дата_выгрузки and
								   Выгрузка_из_ЗОД = @_выгрузка_зод and
								   Отпечаток_времени = @_отпечаток_времени;

		delete from [td].t_raw_Данные_Заявка_СТ_шапки where Код_магазина = @_код_магазина and
		                           Дата_выгрузки = @_дата_выгрузки and
								   Выгрузка_из_ЗОД = @_выгрузка_зод and
								   Отпечаток_времени = @_отпечаток_времени; 
		delete from [td].t_raw_Данные_Заявка_СТ_строки where Код_магазина = @_код_магазина and
		                           Дата_выгрузки = @_дата_выгрузки and
								   Выгрузка_из_ЗОД = @_выгрузка_зод and
								   Отпечаток_времени = @_отпечаток_времени; 
		...
		while (select count(*) from @Данные where Флаг_загрузки is null) > 0 begin
			...
			if @exVer = 1 begin 
				...
				while len(@strData) > 0 begin
					...
					set @Дата_заявки_СТ = cast('20' + left(@currStr, charindex('&', @currStr) - 1) as date);
					set @currStr = right(@currStr, len(@currStr) - charindex('&', @currStr));

					set @Составной_код_заявки_СТ = concat(@_код_магазина, '~', left(@currStr, charindex(';', @currStr) - 1));
					
				insert into
					td.t_fact_Заявка_СТ_шапки(
		...
		while (select count(*) from @Данные_строки where Флаг_загрузки is null) > 0 begin
			...
			if @exVer = 1 begin 
				...
				while len(@strData) > 0 begin
					...
					set @Корректировка = cast(left(@currStr, charindex('&', @currStr) - 1) as decimal) / 1000;
					set @currStr = right(@currStr, len(@currStr) - charindex('&', @currStr));
					...
					set @Заказ_хознужды = cast(left(@currStr, charindex(';', @currStr) - 1) as decimal) / 1000;
					

				insert into
					td.t_fact_Заявка_СТ_строки(
						Составной_код_заявки_СТ,
						Код_товара,
						Остаток,
						Остаток_цех,
