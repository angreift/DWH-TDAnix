	...
	while (select count(*) from @load_pr) > 0 begin
		...
		if (select count(*) from [td].[t_raw_Данные_Заявка_СТ_шапки] with (nolock) where
			...
			if @_дата_выгрузки < (dateAdd(day, - 2, getDate())) begin
				delete from td.t_raw_Данные_Заявка_СТ_строки  where DateExec = @_дата_выгрузки and
																	SubCode = @_код_магазина and
																	TimeStamp = @_отпечаток_времени and
																	DateStart = @_дата_начала_выгрузки and
																	DateEnd = @_дата_конца_выгрузки;
				delete from td.t_raw_Данные_Заявка_СТ_шапки   where DateExec = @_дата_выгрузки and
																	SubCode = @_код_магазина and
																	TimeStamp = @_отпечаток_времени and
																	DateStart = @_дата_начала_выгрузки and
																	DateEnd = @_дата_конца_выгрузки;
		...
		select [Data] from [td].t_raw_Данные_Заявка_СТ_шапки where DateExec = @_дата_выгрузки and
																   SubCode = @_код_магазина and
																   TimeStamp = @_отпечаток_времени and
																   DateStart = @_дата_начала_выгрузки and
																   DateEnd = @_дата_конца_выгрузки;
		insert into @Данные_строки ([data]) 
		select [Data] from [td].t_raw_Данные_Заявка_СТ_строки where DateExec = @_дата_выгрузки and
																	SubCode = @_код_магазина and
																	TimeStamp = @_отпечаток_времени and
																	DateStart = @_дата_начала_выгрузки and
																	DateEnd = @_дата_конца_выгрузки;
