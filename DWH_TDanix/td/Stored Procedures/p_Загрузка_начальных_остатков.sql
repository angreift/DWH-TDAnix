...
BEGIN
	...
	delete from td.[t_raw_Данные_начальных_остатков_из_магазинов] where SubCode is null or 
																		DateExec is null or 
	...
	while (select count(*) from @load_pr) > 0 begin
		...
		delete from td.t_fact_Начальные_остатки where Дата = @_дата_начала_выгрузки and Код_магазина = @_код_магазина
			...
			while len(@strData) > 0 begin
				...
				insert into
					td.t_fact_Начальные_остатки(
						...
						@_дата_начала_выгрузки, -- В этой выгрузке выгружается всегда на один день
						@_код_магазина,
						@Код_склада,
						@Код_товара,
						@Сумма,
