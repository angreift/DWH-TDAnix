...
BEGIN
	...
	while (select count(*) from @load_pr) > 0 begin
		...
		delete from td.t_fact_Товародвижение where Дата >= @_дата_начала_выгрузки and Дата <= @_дата_конца_выгрузки and Код_магазина = @_код_магазина
		...
		select [Data] from [td].t_raw_Данные_товародвижения_из_магазинов where DateExec = @_дата_выгрузки and
																		 	   SubCode = @_код_магазина and
																			   TimeStamp = @_отпечаток_времени and
																			   DateStart = @_дата_начала_выгрузки and
																			   DateEnd = @_дата_конца_выгрузки;
