...
BEGIN
	...
	declare @_код_магазина int, @_дата_выгрузки date, @_выгрузка_зод bit;
	...
	insert into @load_pr
	select distinct 
		Код_магазина, Дата_выгрузки, Выгрузка_из_ЗОД
	from
		[td].[t_raw_Данные_товародвижения_из_магазинов]

	-- Преобразуем по циклу
	while (select count(*) from @load_pr) > 0 begin
		Set @_код_магазина = (Select top 1 Код_магазина from @load_pr order by Код_магазина asc, Дата_выгрузки asc, выгрузка_ЗОД desc);
		Set @_дата_выгрузки = (Select top 1 Дата_выгрузки from @load_pr where Код_магазина = @_код_магазина order by Код_магазина asc, Дата_выгрузки asc, выгрузка_ЗОД desc);
		Set @_выгрузка_зод = (Select top 1 выгрузка_ЗОД from @load_pr where Код_магазина = @_код_магазина and Дата_выгрузки = @_дата_выгрузки order by Код_магазина asc, Дата_выгрузки asc, выгрузка_ЗОД desc)
	
		Delete from @load_pr where Код_магазина = @_код_магазина and
		                           Дата_выгрузки = @_дата_выгрузки and
								   выгрузка_ЗОД = @_выгрузка_зод
		...
		while (select count(*) from td.t_raw_Данные_товародвижения_из_магазинов 
			...
			if @exVer = 1 begin 
				...
				set @strData = substring(@strData, 14, len(@strData) - 13);
