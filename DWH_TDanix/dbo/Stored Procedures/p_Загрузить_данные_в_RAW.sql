...
CREATE PROCEDURE [dbo].[p_Загрузить_данные_в_RAW]
	...
	@DateEndStr nvarchar(6), @Data nvarchar(max)
AS BEGIN
	...
	Set @SQL = 'Insert into @Table (Дата_время, Version, Subcode, DateExec, TimeStamp, DateStart, DateEnd, Data) 
				values (GetDate(), @Version, @Subcode, @DateExec, @TimeStamp, @DateStart, @DateEnd, @Data)';
	Set @ParamDef = '@Version int, @SubCode int, @DateExec date, @Timestamt int, @DateStart date,
					 @DateEnd date, @Data nvarchar(max)';
	
	exec sp_executesql @SQL, @ParamDef, @Version = @Version, @SubCode = @SubCode, @DateExec = @DateExec, @TimeStamp = @TimeStamp,
					   @DateStart = @DateStart, @DateEnd = @DateEnd, @Data = @Data;
END
GO