...
insert into rprt_cw.t_fact_Факты_потерь(Дата, Код_магазина, Код_товара, Сумма, Количество) --загрузка Фактов потерь за неделю
select тд.Дата, тд.Код_магазина, тд.Код_товара, sum(тд.СуммаЗакупаСоСклада), sum(Количество)
FROM [DWH].[dbo].[v_fact_Товародвижение] as ТД
	left join dbo.t_dim_Товары Т on т.Код_товара=тд.Код_товара
where (тд.Дата between @dateStart and @dateEnd) and Количество!=0 and  Количество is not null and Код_причины in(12,42,175,18,17) and т.Код_группы not in(5815,24466,17993,37,39132)
